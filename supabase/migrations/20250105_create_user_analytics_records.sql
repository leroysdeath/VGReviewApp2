-- STEP 1: Check if user_analytics table exists and has the user_id column
SELECT
...
-- STEP 2: Create analytics records for ALL existing users
...
SELECT
  ...
  CASE
    WHEN (SELECT COUNT(*) FROM "user") = (SELECT COUNT(*) FROM user_analytics)
    THEN 'SUCCESS - All users have analytics records'
    ELSE 'WARNING - Mismatch in record counts'
  END as verification;

-- STEP 4: Now populate the analytics data
...
SET
  ...
  profile_completion_score = GREATEST(0, LEAST(100, (
...
-- STEP 5: Generate referral codes for users who don't have them
...
-- STEP 6: Set early adopter flag for first 1000 users
UPDATE user_analytics ua
...
-- STEP 7: Display final summary statistics
WITH stats AS (
  SELECT
