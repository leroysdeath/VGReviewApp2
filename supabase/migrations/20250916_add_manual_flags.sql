-- Migration: Add Manual Flags for Admin Override
-- Date: 2025-09-16
-- Purpose: Add greenlight and redlight flags for manual admin control over filtering
...
-- Add comments
COMMENT ON FUNCTION set_game_flag(INTEGER, TEXT, TEXT, UUID) IS 'Sets manual flags for games with admin override capability';
COMMENT ON FUNCTION get_flagged_games_summary() IS 'Returns summary statistics of manually flagged games';
COMMENT ON VIEW game_flags_admin IS 'Admin view of all manually flagged games with conflict detection';
...
-- RLS policies for flag management (adjust based on your admin system)
-- Note: You may want to restrict these to admin users only
CREATE POLICY game_flags_admin_select ON game 
FOR SELECT USING (true); -- Adjust this to admin-only if needed

CREATE POLICY game_flags_admin_update ON game 
FOR UPDATE USING (true); -- Adjust this to admin-only if needed