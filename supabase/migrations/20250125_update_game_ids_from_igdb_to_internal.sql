...
SELECT 
    ...
    r.id,
    r.user_id,
    r.game_id as new_internal_id,
    g.name as game_name,
    g.igdb_id as original_igdb_id
FROM rating r
...
-- Note: Foreign key constraints already exist:
-- rating table has: fk_rating_game (game_id REFERENCES game(id) ON DELETE CASCADE)
-- game_progress table has: fk_game_progress_game (game_id REFERENCES game(id) ON DELETE CASCADE)
-- No need to create new constraints

-- Note: Indexes idx_rating_game_id and idx_game_progress_game_id already exist, no need to create them

-- Optional: Create a rollback script (commented out)
-- To rollback, you would need to restore from backup or manually update back to IGDB IDs:
/*
-- Rollback rating table - convert back to IGDB IDs
UPDATE rating r
SET game_id = g.igdb_id
FROM game g
WHERE r.game_id = g.id;

-- Rollback game_progress table - convert back to IGDB IDs
UPDATE game_progress gp
SET game_id = g.igdb_id
FROM game g
WHERE gp.game_id = g.id;

-- Note: Foreign key constraints would still work since game.igdb_id values exist in game.id column
*/
