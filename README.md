...
This project includes comprehensive testing to ensure high quality and reliability:

### Unit Tests

Unit tests are written using Jest and React Testing Library. Run them with:
...
Integration tests use Cypress for component and end-to-end testing:
...
Accessibility tests are integrated into both Jest and Cypress tests using axe-core:

```bash
# Run accessibility tests with Jest
npm run test:a11y

# Accessibility is also checked in Cypress tests
```

### Visual Regression Testing

Visual regression tests use Storybook and Chromatic:
...
# Run Chromatic tests (requires API token)
npm run chromatic
```

### Performance Testing

Performance testing is integrated into Cypress using Lighthouse:

```bash
# Performance tests are run as part of E2E tests
npm run test:e2e
```

### Code Quality

Code quality is enforced using ESLint, Prettier, and Husky pre-commit hooks:
