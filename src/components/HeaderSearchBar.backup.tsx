...
export const HeaderSearchBar: React.FC<HeaderSearchBarProps> = ({ 
  ...
  const { 
    ...
    navigateToSearch,
    clearSearch,
    searchState  // Add searchState to get the same results as SearchResultsPage
  } = useGameSearch();
  ...
  // Enhanced quick search for games using the same useGameSearch.quickSearch method
  const performGameSearch = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (enableCache) {
        ...
        if (cached && Date.now() - cached.timestamp < 5 * 60 * 1000) { // 5 minutes
          ...
          if (import.meta.env.DEV) {
            console.log('🚀 HeaderSearchBar - Cache hit:', query);
          }
          return;
        }
      }

      // Use the EXACT SAME results from the shared searchState that SearchResultsPage uses
      if (import.meta.env.DEV) {
        console.log('🔍 HeaderSearchBar - Using shared searchState, triggering main search');
      }
      
      // Trigger the main search which will populate searchState.games
      await searchGames(query);
      
      // Use the results from searchState (same as SearchResultsPage)
      const searchResults = searchState.games;
      
      if (searchResults && Array.isArray(searchResults)) {
        const limitedResults = searchResults.slice(0, maxSuggestions);
        ...
        if (import.meta.env.DEV) {
          console.log('🔍 HeaderSearchBar - Direct coordination results:', query, limitedResults.length, 'results');
          console.log('🎯 HeaderSearchBar - First 5 results:', limitedResults.slice(0, 5).map(r => r.name));
          console.log('🔤 HeaderSearchBar - Search expanded queries:', searchResult.context.expandedQueries);
          console.log('🎯 HeaderSearchBar - Search intent:', searchResult.context.searchIntent);
        }
      } else {
        setSuggestions([]);
        setShowSuggestions(false);
      ...
      console.error('HeaderSearchBar - quickSearch failed:', error);
      setSuggestions([]);
