...
export const GameActionSheet: React.FC<GameActionSheetProps> = ({
  ...
  const IconStrip = () => (
    <button
      onClick={handleOpen}
      className="md:hidden flex items-center gap-2 px-3 py-2 bg-gray-800 rounded-full hover:bg-gray-700 transition-all active:scale-95"
    ...
    >
      ...
      {/* Started Icon */}
      <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${
        isStarted ? 'bg-blue-600' : 'bg-gray-700'
      }`}>
      ...
      {/* Review Icon */}
      <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${
        userHasReviewed ? 'bg-purple-600' : 'bg-gray-700'
      }`}>
