  ...
  if (activeTab === 'top5') {
    ...
    return (
      <div>
        ...
        {/* Mobile Version */}
        <div className={forceMobileView ? 'space-y-4' : 'md:hidden space-y-4'}>
          {top5Games.map((game, index) => (
            <Link
              key={game.id}
              to={`/game/${game.id}`}
  ...
  if (activeTab === 'top50') {
    ...
    return (
      <div>
        ...
        <div className={forceMobileView ? 'space-y-3' : 'md:hidden space-y-3'}>
          {top50Games.map((game, index) => (
            <Link
              key={game.id}
              to={`/game/${game.id}`}
      ...
      <div>
        ...
        <div className="space-y-4 sm:space-y-6">
          {sortedReviews.map((review) => {
            ...
            return (
              <div key={review.id} className={`flex gap-4 p-4 sm:p-6 bg-gray-800 rounded-lg ${forceMobileView ? 'flex-col' : 'flex-col sm:flex-row'}`}>
                <Link to={`/game/${game?.id}`} className="flex-shrink-0 self-start">
                  <img
                    src={game?.coverImage}
                    alt={game?.title}
                    className={`object-cover rounded ${forceMobileView ? 'w-16 h-20 mx-auto' : 'w-16 h-20 sm:w-20 sm:h-28 mx-auto sm:mx-0'}`}
                  />
                </Link>
                <div className="flex-1">
                  <div className={`gap-2 mb-2 ${forceMobileView ? 'flex flex-col' : 'flex flex-col sm:flex-row sm:items-center'}`}>
                    <Link
                      to={`/game/${game?.id}`}
